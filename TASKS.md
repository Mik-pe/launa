...
- [ ] **Add `Clock` trait to `launa-hal` for testable time**: `app/` uses `embassy_time::Instant::now()` directly in ~15 call sites across `pump_timer.rs`, `mqtt_client.rs`, `command_tracker.rs`, `heap_monitor.rs`, and `main.rs`. Meanwhile `launa-sim/src/controller.rs` maintains its own tick-based virtual clock (`PumpTimer` with `at_tick: u64`) that diverges from real firmware logic. Add a `Clock` trait to `launa-hal` with `fn now(&self) -> Instant` / `fn elapsed(&self, earlier: Instant) -> Duration`. Provide two impls: `EmbassyClock` in `app/` (wraps `embassy_time::Instant::now()`), `VirtualClock` in `launa-sim` / test code (tick-based, manually advanceable). This unifies timer logic across firmware and simulation, enables deterministic time-dependent tests (pump auto-off, command timeouts, periodic publishing), and removes the duplicate `PumpTimer` in the simulator. Scope: only `Instant`-comparison logic (timeouts, intervals, durations) -- `Timer::after().await` async scheduling stays as-is.
