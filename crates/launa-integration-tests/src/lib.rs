...
mod tests {
    ...
    //
    // Now uses the extracted types from launa-core and launa-mqtt instead
    // of a duplicated local implementation.
    // ========================================================================

    #[test]
    fn test_remote_log_buffer_push_and_drain() {
        let mut buf = launa_core::RemoteLogBuffer::new();
        buf.init();
    ...
    fn test_remote_log_buffer_ring_overwrite() {
        ...
        for i in 0..launa_core::REMOTE_LOG_BUF_SIZE + 4 {
            buf.push("warn", &format!("msg {}", i), i as u64 * 100);
        }

        assert_eq!(buf.len(), launa_core::REMOTE_LOG_BUF_SIZE);

        let entries = buf.drain();
        assert_eq!(entries.len(), launa_core::REMOTE_LOG_BUF_SIZE);
    ...
    fn test_remote_log_buffer_truncation() {
        ...
        assert!(entries[0].message.len() <= launa_core::MAX_LOG_MESSAGE_LEN);
    }

    #[test]
    fn test_remote_log_entry_to_json() {
        let entry = launa_mqtt::RemoteLogEntry {
            level: "warn",
            message: String::from("Temperature high"),
            timestamp_ms: 12345,
        };
        let json = launa_mqtt::log_entry_to_json(&entry);
