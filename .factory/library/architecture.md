...
- **launa-protocol** — Balboa spa RS-485 protocol: frame encode/decode (CRC-8, 0x7E-delimited length-field framing), message dispatch, status/config/fault/filter/info parsing, command encoding, registration state machine. **Dispatcher catch-all:** The dispatcher has a catch-all arm `[_, 0xBF] => IncomingMessage::Ready` that matches any first byte with second byte 0xBF. When writing dispatcher tests for Unknown message types, use a second byte != 0xBF (e.g., 0xCD) — otherwise the frame dispatches as Ready instead of Unknown.
...
- FrameDecoder: CRC-8 + 0x7E-delimited length-field framing, configurable max_buffer_size (default 512) with overflow protection
